	...
	while (i + map->piece_y < map->coordinate_y)
	{
		...
		while (j + map->piece_x < map->coordinate_x)
		{
			if (check_collision(map, i, j))
			{
				...
				if (current_sum < map->l_sum)
				{
					map->l_sum = current_sum;
					map->res_x = j;
					map->res_y = i;
				}
			}
...
int		check_collision(t_map *map, int i, int j)
{
	...
	while (a < map->piece_y)
	{
		...
		while (b < map->piece_x)
		{
			if (map->field[i + a][j + b] == PLAYER
				&& map->figure_repr[a][b] == '*')
				coll++;
			if (map->field[i + a][j + b] == ENEMY
				&& map->figure_repr[a][b] == '*')
				return (0);
			b++;
		}
...
int		figure_sum(t_map *map, int i, int j)
{
	...
	while (a < map->piece_y)
	{
		...
		while (b < map->piece_x)
		{
			if (map->figure_repr[a][b] == '*' &&
			map->field[i + a][j + b] != PLAYER)
